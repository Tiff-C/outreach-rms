...
class Referral(models.Model):
    """
    A class to define the referral model.
    Contains choices for 'student status' and field types for information
    required to create a new referral.
    """
    ...
    phone = models.CharField("Contact Phone", max_length=15, blank=True, null=True)
...
class Student(Referral):
    """
    Student model inherits from the Referral model using Multi-Table Inheritance.
    """
    dob = models.DateField("Date of birth", blank=True, default=None)
    student_num = models.CharField("Student Number", max_length=8)
    enrol_url = models.URLField("Link to myPortal Page", blank=True)
    current_student = models.BooleanField("Current Student", default=True)
    progression = models.JSONField("Progression Details", blank=True)
